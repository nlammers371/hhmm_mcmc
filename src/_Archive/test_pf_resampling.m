...
mcmcInfo.seq_length = 100; % length of simulated traces in time steps
...
if mcmcInfo.upsample_factor > 1
    usf = mcmcInfo.upsample_factor;
    trueParams.nSteps = trueParams.nSteps*usf;
    trueParams.A = expm(trueParams.R/usf*trueParams.tres);
    sl_orig = trueParams.seq_length;
    trueParams.seq_length = trueParams.seq_length*usf-usf+1;
    trueParams.alpha = trueParams.alpha*usf;
    % interpolate
    obf = trueParams.observed_fluo;
    tv_orig = trueParams.tres:trueParams.tres:trueParams.tres*sl_orig;
    tv_new = trueParams.tres:trueParams.tres/usf:trueParams.tres*sl_orig;
    trueParams.observed_fluo = interp1(tv_orig',obf,tv_new','linear','extrap');
    
    mcmcInfo.seq_length = mcmcInfo.seq_length*usf-usf+1;
end
