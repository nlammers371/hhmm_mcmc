...
%%
a21_vec = reshape(mcmcInfo.A_inf_array(2,1,:,si(1)),[],1)/20*60; % using first order rates for illustrative purposes
a12_vec = reshape(mcmcInfo.A_inf_array(1,2,:,si(1)),[],1)/20*60;
grid_size = 100;
t_grid = 1:length(a12_vec);

close all
param_space_fig = figure;
% cmap = flipud(brewermap(length(t_grid),'Spectral'));
cmap = brewermap([],'Set2');
...
% make density plot
y_axis = linspace(0.3,1,grid_size)*3;
x_axis = linspace(0.1,.5,grid_size)*3;
[x_grid, y_grid] = meshgrid(x_axis,y_axis);
[point_density, point_grid,bw] = ksdensity([a21_vec a12_vec],[x_grid(:),y_grid(:)]);
...
plot(a21_vec,a12_vec,'Color',[0 0 0 0.1],'LineWidth',1)
scatter(a21_vec,a12_vec,50,'MarkerFaceColor',cmap(2,:),'MarkerEdgeColor','k','LineWidth',1,'MarkerFaceAlpha',0.25,'MarkerEdgeAlpha',0.1);
s = scatter(0,0,100,'MarkerFaceColor',cmap(2,:),'MarkerEdgeColor','k','LineWidth',1,'MarkerFaceAlpha',1,'MarkerEdgeAlpha',1);
...
p1 = plot(repelem(trueParams.A(2,1),n_mcmc_steps)*3,linspace(yl(1),yl(2),n_mcmc_steps),'-.k','LineWidth',2);
plot(linspace(xl(1),xl(2),n_mcmc_steps),repelem(trueParams.A(1,2),n_mcmc_steps)*3,'-.k','LineWidth',2)


legend([s p1],'burstMCMC samples','true values','Location','southeast')
...
xlabel('on rate (k_{on})')
ylabel('off rate (k_{off})')


param_space_fig.InvertHardcopy = 'off';
set(gcf,'color','w');

saveas(param_space_fig,[figPath 'param_space_fig.png'])
saveas(param_space_fig,[figPath 'param_space_fig.pdf'])
