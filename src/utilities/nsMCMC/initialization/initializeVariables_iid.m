function mcmcInfo = initializeVariables_iid(mcmcInfo)
    ...
    for n = 1:n_chains_eff    
        pi0_temp = [10 5 1];%ones(1,mcmcInfo.nStates);
        pi0_samp = drchrnd(pi0_temp,1);             
        mcmcInfo.pi0_alpha(n,:) = pi0_samp;   
    ...
    mcmcInfo.v0 = [zeros(n_chains_eff,1) 1.5*v2*rand()];
    if mcmcInfo.nStates==3
         mcmcInfo.v0(:,end+1) = v2*rand()*1.5;
    end
    mcmcInfo.M0 = eye(mcmcInfo.nStates)*0.3;    
    ...
    mcmcInfo.v_curr(mcmcInfo.v_curr<0) = 0;