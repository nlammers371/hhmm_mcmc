        ...
        if n == 1 || ~mcmcInfo.ensembleInferenceFlag
            if ~mcmcInfo.rateSamplingFlag
                ...
                if nStates == 2
                    ...
                    kon_total = pi0_counts(1,1,n);
                    koff_tr = tr_counts(1,2,n);
                    koff_total = pi0_counts(1,2,n);
                    % sample from gamma 
                    kon = gamrnd(mcmcInfo.alpha_kon+kon_tr,1/(mcmcInfo.beta_kon+kon_total));
                    koff = gamrnd(mcmcInfo.alpha_koff+koff_tr,1/(mcmcInfo.beta_koff+koff_total));
                ...
                elseif nStates == 3
                    ...
                    kon_tr = tr_counts(2,1,n);
                    kon_total = pi0_counts(1,1,n);
                    koff_tr = tr_counts(2,3,n);
                    koff_total = pi0_counts(1,3,n);
                    % sample from gamma 
                    kon = gamrnd(2*mcmcInfo.alpha_kon+kon_tr,1/(mcmcInfo.beta_kon+kon_total));
                    koff = gamrnd(2*mcmcInfo.alpha_koff+koff_tr,1/(mcmcInfo.beta_koff+koff_total));
                    % now middle
                    mid_tr = pi0_counts(1,2,n) - tr_counts(2,2,n);
                    mid_rate = gamrnd(mcmcInfo.alpha_koff+mcmcInfo.alpha_kon+mid_tr,1/(2*mcmcInfo.beta_koff+mid_tr));
                    kon_frac = tr_counts(3,2)/(tr_counts(3,2)+tr_counts(1,2));
                    kon_mid = mid_rate*kon_frac;
                    koff_mid = mid_rate*(1-kon_frac);
                    % make rate matrix
                    Q_init = [-kon koff_mid 0; kon -mid_rate koff; 0 kon_mid -koff];
                    mcmcInfo.Q_curr(:,:,n) = Q_init/mcmcInfo.tres*mcmcInfo.upsample_factor;
