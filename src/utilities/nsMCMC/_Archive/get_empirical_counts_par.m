function mcmcInfo = get_empirical_counts_par(mcmcInfo)
    ...
    for i = 1:length(unique_indices)
        mcmcInfo.transition_count_array(unique_indices(i)) = transition_counts(i);
    end
    ...
    linear_indices = (mcmcInfo.sample_chains(1,:,:)-1)*n_chains + (1:n_chains);
    logL_pi0 = pi0_log(linear_indices);
    if n_chains == 1
        logL_pi0 = permute(logL_pi0,[1,3,2]);
    end
