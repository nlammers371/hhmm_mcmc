function mcmcInfo = update_hmm_parameters_v3(mcmcInfo)    
    ...
    for c = 1:n_chains
        for n = 1:n_traces        
            ...
            for m = 1:nStates
                % record counts
                state_counts = convn(coeff_MS2(:,n),mcmcInfo.sample_chains(:,ref_chain_ids(c),n)==m,'full');            
...
%     end
    for c = 1:n_chains
        ...
        M = ((F_array(:,:,c)'*F_array(:,:,ref_chain_ids(c))))  + 1e-4;    
        b = ((F_array(:,:,c)'*y_array(:,ref_chain_ids(c))));
