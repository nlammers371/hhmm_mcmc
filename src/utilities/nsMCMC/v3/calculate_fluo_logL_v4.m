function logL_fluo = calculate_fluo_logL_v4(mcmcInfo,sample_chains_dummy,observed_fluo_dummy,indexArray)
                  
    % extrac useful parameters
    seq_length_dummy = size(sample_chains_dummy,1);%mcmcInfo.seq_length;
...
%     v_ref = permute(repmat(mcmcInfo.v_curr',1,n_chains,n_traces,1),[4,2,3,1]);
    ...
    linIndexArrayFull = indexArrayFull + mcmcInfo.chain_id_ref*seq_length_dummy + mcmcInfo.temp_id_ref*seq_length_dummy*n_chains;  
    
    % extract states
    naive_states = sample_chains_dummy(linIndexArrayFull);
    ...
    initiation_fragment(nSteps,:,:,:) = repmat(v_perm,1,1,n_temps);%repmat(mcmcInfo.sample_chains_slice(prevInd:postInd,:),1,1,nStates);           
    ...
    % get differences   
    linIndexArrayFluo = indexArrayFull(nSteps:end,:,:);% +  mcmcInfo.temp_id_ref*seq_length_dummy;
    ref_fluo = repmat(observed_fluo_dummy(linIndexArrayFluo),1,1,1,nStates);
    ...
    sigma_ref = repmat(sigma_curr,nSteps,1,n_temps,nStates);
