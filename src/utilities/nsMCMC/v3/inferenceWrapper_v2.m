function mcmcInfo = inferenceWrapper_v2(mcmcInfo)
    ...
    for step = 2:mcmcInfo.n_mcmc_steps %mcmcInfo.n_mcmc_steps   
        % annealing stuff
        mcmcInfo.tempGradVec = mcmcInfo.tempGradArray(mcmcInfo.step,:);    
        if mcmcInfo.annealingSigmaFlag
            mcmcInfo.sigma_curr = mcmcInfo.sigma_curr .* mcmcInfo.tempGradVec';
            mcmcInfo.tempGradVec = ones(size(mcmcInfo.tempGradVec));
        end
